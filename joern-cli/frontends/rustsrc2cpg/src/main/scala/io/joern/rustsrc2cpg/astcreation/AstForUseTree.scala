...
trait AstForUseTree(implicit schemaValidationMode: ValidationMode) { this: AstCreator =>
  ...
  private def astForUsePath(filename: String, parentFullname: String, usePathInstance: UsePath): Ast = {
    val code = codeForUsePath(filename, parentFullname, usePathInstance)
    val node = newImportNode(code, code, code, usePathInstance).isExplicit(true)
    Ast(node)
  }
  ...
  private def astForUseRename(filename: String, parentFullname: String, useRenameInstance: UseRename): Ast = {
    val code = codeForUseRename(filename, parentFullname, useRenameInstance)
    val node = newImportNode(code, useRenameInstance.ident, useRenameInstance.rename, useRenameInstance)
      .isExplicit(true)
      .explicitAs(true)
    Ast(node)
  }
  ...
  private def astForUseGroup(filename: String, parentFullname: String, useGroupInstance: UseGroup): Ast = {
    val code = codeForUseGroup(filename, parentFullname, useGroupInstance)
    val node = newImportNode(code, code, code, UnknownAst()).isExplicit(true)
    Ast(node)
  }
}
...
trait CodeForUseTree(implicit schemaValidationMode: ValidationMode) { this: AstCreator =>
  ...
  def codeForUsePath(filename: String, parentFullname: String, usePathInstance: UsePath): String = {
  ...
  def codeForUseName(filename: String, parentFullname: String, ident: Ident): String = {
    ident.toString
  }

  def codeForUseRename(filename: String, parentFullname: String, useRenameInstance: UseRename): String = {
    s"${useRenameInstance.ident} as ${useRenameInstance.rename}"
  }

  def codeForUseGroup(filename: String, parentFullname: String, useGroupInstance: UseGroup): String = {
